...
<form action ="" method="post">
	...
	<div style="position: absolute; top: 30%; left: 30%;background-color:white;border-radius: 10% 10%; width:40%; height:60%;">
		
		<h3 style="position: absolute; top: 0%; left: 5%; font-size:200%;" >Notes</h3>
 
 <a  style="position:absolute; top:15%; left:30%; font-size:150%;"  cellspacing="17"  >Notes </a>
...
<?php 
		...
		$query = "SELECT nots
			FROM  `notes` where student_id = '$_SESSION[login_user]'";

		$result = mysqli_query($l, $query);

        echo '<nav  class="table-wrapper-scroll-y3 my-custom-scrollbar3" style="position: relative; margin-top:-15%; left:12%;">';
        echo '<table style="position:relative; background:#F7DC6F; width:20%;  border: 1px solid #EC7063; left:25%; margin-top:10%; font-size:80%;" cellspacing="2"  >';

		while($row = mysqli_fetch_array($result)) {
        
		echo "<tr>";
		
		echo "<td >"; echo $row['nots'];  echo "</td>";
